...
def relative_error_in_total_energy(y_true, y_pred):
    sum_output = np.sum(y_pred)
    sum_target = np.sum(y_true)
    return float((sum_output - sum_target) / (max(sum_output, sum_target)+0.1))
...
METRICS = {
    'classification': {
        ...
        'confusion_matrix': metrics.confusion_matrix # [TN, FP, FN, TP]
        },
    'regression': {
...
class Metrics():
    def __init__(self, APPLIANCES, target_series, pred_series, acc_threshold = 30):
    ...
    def init_parameters(self, APPLIANCES, target_series, pred_series, acc_threshold):
        ...
        self.target_on_off = self.target > acc_threshold
        self.pred_on_off = self.pred > acc_threshold
    ...
    def compute_metrics(self):
        for batch_idx in range(self.NUM_SEQ_PER_BATCH):
    ...
    def compute_appliance_metrics(self, target_batch_series, pred_batch_series, appliance_idx, category):
        ...
        for metrics_type, metrics in METRICS[category].iteritems():
            if category == 'classification' and metrics_type != 'accuarcy':
                result = metrics(y_true=target_series, y_pred=pred_series, labels=[False, True])
            else:
    ...
    def print_metrics(self):
        for appliance_idx in range(self.num_appliance):
            ...
            for category in METRICS.keys():
                print('-'*5, label, 'validation of', category, 'metrics', '-'*5)
