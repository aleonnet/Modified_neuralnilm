...
class DataPipeline(object):
    def __init__(self, sources, num_seq_per_batch,
        ...
        self.time_processing = none_to_list(time_processing)
    ...
    def get_batch(self, fold='train', enable_all_appliances=False,
        ...
        try:
            ...
            batch.after_processing.input, input_metadata = self.apply_processing(
                batch.before_processing.input, 'input')
            batch.after_processing.time, time_metadata = self.apply_processing(
                batch.before_processing.time, 'time')
            batch.after_processing.target, target_metadata = self.apply_processing(
    ...
    def apply_inverse_processing(self, data, net_input_or_target):
        ...
        for step in reversed_processing_steps:
            try:
                data = np.array([step.inverse(power_series) for power_series in data])
            except AttributeError:
                pass
        return data

    def _get_processing_steps(self, net_input_or_target):
        assert net_input_or_target in ['input', 'target', 'time']
    ...
    def report(self):
        ...
        report['time_processing'] = [
            processor.report() for processor in self.time_processing]
